(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root['SimditorMarkdown'] = factory(root["jQuery"],root["Simditor"],root["toMarkdown"],root["marked"]);
  }
...
SimditorMarkdown = (function(superClass) {
  ...
  SimditorMarkdown.prototype._init = function() {
    ...
    this.editor.formatter._allowedAttributes = $.extend({
      th: ['align'],
      td: ['align']
    }, this.editor.formatter._allowedAttributes);
    this.editor.formatter._allowedTags = $.merge(['em'], this.editor.formatter._allowedTags);
    this.converters = [
      {
        filter: function(node) {
          return node.nodeName === 'PRE' && node.children[0].nodeName === 'CODE';
        },
    ...
    this.markdownChange = this.editor.util.throttle((function(_this) {
      return function() {
        ...
        _this._convert();
        _this.editor._placeholder();
        return _this.editor.trigger('simditor-markdown-valuechanged');
      };
    })(this), 200);
  ...
  SimditorMarkdown.prototype._convert = function() {
    ...
    markdownText = markdownText.replace(/(<(?:th|td)\s+[^>]*style="[^"]*text-align:\s*(\w+)[^"]*"[^>]*)(>)/g, '$1 align="$2"$3');
